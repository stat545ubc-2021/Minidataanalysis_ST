...
The boxplot I created before is an improvement from before because I can now 
...
whereas the control for the Duke variety is the Duke pollen donor. But ggplot
organizes the boxplots in alphabetical order by default. I want the boxplots
that represent the control treatments to go first in each plot (this is already
the case for the Bluecrop variety boxplots but not for the Duke variety boxplots). 

####Alternatively, I could use fct_explicit_na to see how many NAs exist in a variable.
...
It may be helpful to combine both varieties and create a new dummy variable 
column so each row of data is classified as either self-crossed or out-crossed. 
Combining the data like this could increase the chances of fitting the data to a 
linear model but perhaps unfairly so. Blueberry variety can have a significant 
effect on blueberry weight, TSS, pH, and TA, so removing this factor from the 
model could hurt it more than help. Regardless, I will explore what a boxplot
would look like if I combined this data.

```{r}
#First, I need to create the new column (self-crossed vs out-crossed)
alldata<- Pilot %>%
...
#Now I can create the next boxplot
#Note: the code used to remove the a-axis label was obtained from https://www.datanovia.com/en/blog/ggplot-axis-labels/#:~:text=Remove%20the%20x%20and%20y,x%20%3D%20element_blank())%20.
